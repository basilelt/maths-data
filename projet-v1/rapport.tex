...
Le script \texttt{projet-v1.py} met en œuvre ce qui suit :
\begin{itemize}
    ...
    \item \textbf{Implémentation from scratch} : La classe \texttt{CustomLogisticRegression}.
...
\begin{itemize}
    \item \textbf{Modèle Custom (sans régularisation)} : Accuracy = 96.94\%
    \item \textbf{Modèle Scikit-learn} : Accuracy = 97.50\%
    \item \textbf{Modèle Custom (avec régularisation L2)} : Accuracy = 97.50\%
\end{itemize}
L'ajout de la régularisation L2 a permis à notre modèle d'égaler les performances de l'implémentation de scikit-learn.
